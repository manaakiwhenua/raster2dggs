...
The README figures are generated by [docs/imgs/generate_figures.py](docs/imgs/generate_figures.py), which fetches its source rasters from their public homes and runs the CLI — rerun it (with `matplotlib` installed and the `gdal_translate` binary on `PATH`) whenever output or behaviour they depict changes.
