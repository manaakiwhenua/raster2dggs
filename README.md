...
![Aerial imagery indexed to H3 at two resolutions, with the tabular one-row-per-cell output, the other supported DGGS grids on the same window, and the hive-partitioned Parquet it all produces](docs/imgs/raster2dggs-example.png "Aerial imagery indexed to H3 at two resolutions, alongside the tabular output, the other supported DGGS grids, and the hive-partitioned Parquet on disk")
...
![The three sampling strategies compared at a mask boundary: point sampling, area-weighted overlay, and bilinear resampling](docs/imgs/sampling-strategies.png "The three sampling strategies compared at a mask boundary")

The differences concentrate at edges — a nodata/mask boundary, or wherever cell size and pixel size diverge: `--point` takes the one pixel under each cell centre, `--overlay` averages every valid pixel a cell touches (weighted by intersection area), and `--sample` interpolates a value at the cell centre from a kernel of neighbouring pixels.
...
![One A5 cell exploded: every --point, --overlay and --sample output schema for the same cell, beside the struct each one writes](docs/imgs/struct-outputs.png "What a cell holds when many pixels fall inside")

These schemas are easier to read as output than as type signatures. The figure explodes a single A5 resolution 14 cell — 52 pixel centres inside it, 69 pixels touched — and shows what each mode writes into its column, for a continuous surface and for its classification. Two things it makes concrete: `--overlay histogram` bins continuous data into `struct<left, right, counts>`, but with no `--hist-bins`/`--hist-width` it counts *exact values* into the `struct<values, counts>` above, one bin per class present; and `-d`/`--decimals` rounds every number a schema emits — struct members, list elements and fractions alike — so `-d 0` on a classified raster collapses every fraction to 0 or 1.
...
`-co`/`--compact` losslessly replaces every complete group of sibling cells that share identical values with their parent cell, recursively, producing a mixed-resolution output that is exact but much smaller wherever the data is locally uniform. Categorical rasters compact dramatically; continuous rasters barely at all (adjacent aggregates rarely coincide).

![WorldCover land cover indexed to rHEALPix resolution 12, before and after compaction](docs/imgs/compaction-example.png "The same classified raster before and after --compact")
...
The README figures are generated by [docs/imgs/generate_figures.py](docs/imgs/generate_figures.py), which fetches its source rasters from their public homes and runs the CLI — rerun it (with `matplotlib` installed and the `gdal_translate` binary on `PATH`) whenever output or behaviour they depict changes. The output-schema figure is the exception: it needs a continuous field with known units and a controlled spread, so it synthesises one with this repo's own neutral landscape model ([make_samples.py](make_samples.py)) and classifies the same field, seeded so the figure is reproducible.
